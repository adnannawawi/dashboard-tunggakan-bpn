...
// Jalur penyimpanan file JSON di folder internal proyek / tmp
const getFilePath = () => {
  try {
    const defaultPath = path.join(process.cwd(), "data_berkas_cache.json");
    // Tes apakah direktori bisa ditulis
    fs.accessSync(process.cwd(), fs.constants.W_OK);
    return defaultPath;
  } catch (e) {
    // Jika root directory read-only, gunakan folder /tmp
    return path.join("/tmp", "data_berkas_cache.json");
  }
};
